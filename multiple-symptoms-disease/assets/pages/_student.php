...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['send_report_id'])) {
        $pid = intval($_POST['send_report_id']);
        $stmt = $conn->prepare("UPDATE student_accepted_patients SET report_sent = 1 WHERE patient_id = ? AND student_email = ?");
        $stmt->bind_param("is", $pid, $student_email);
        $stmt->execute();
        exit;
    }
}
